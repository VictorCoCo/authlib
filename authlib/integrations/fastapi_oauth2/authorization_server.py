...
from fastapi import JSONResponse, Response, Request
...
class AuthorizationServer(_AuthorizationServer):
    ...
    def save_token(self, token, request: Request):
        return self._save_token(token, request)

    def get_error_uri(self, request: Request, error):
        if self._error_uris:
            uris = dict(self._error_uris)
            return uris.get(error.error)

    def create_oauth2_request(self, request: Request):
        return create_oauth_request(request, OAuth2Request)

    def create_json_request(self, request: Request):
        return create_oauth_request(request, HttpRequest, True)

    def handle_response(self, status_code, payload, headers):
        if isinstance(payload, dict):
            return JSONResponse(payload, status=status_code, headers=headers)
